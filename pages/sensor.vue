...
div
  .body.container
    ...
    form
      ...
      .row
        .col-4.col-md-3.mt-2 Lens focal length:
        .col-8.col-md-3.mt-2
          input.form-control.inp4#focallength(
            type="number"
            v-on:change="CalculateSensor"
          )
        .col-4.col-md-3.mt-2 Desired aspect ratio:
        .col-8.col-md-3.mt-2
...
export default {
  ...
  methods: {
    CalculateSensor: function() {
      ...
      if (focalLength == "") {
        this.output = '<strong class="text-warning">Please fill in focal length number</strong>'
      } else if (focalLength !== "") {

        // Find the dataof fromCamera and toCamera first with lodash
        let fromCameraData = $lodash.chain(this.CameraDataList)
                                      .map(o => $lodash.omit(o, ['sensor_width', 'sensor_height', 'sensor_diagonal', 'aspectratio_opengate','aspectratio_178','active_sensor_height_178']))
                                      .find(['camera', fromCamera])
                                      //.map(o => $lodash.omit(o, ['id', 'camera']))
                                      .value();
        let toCameraData = $lodash.chain(this.CameraDataList)
                                      .map(o => $lodash.omit(o, ['sensor_width', 'sensor_height', 'sensor_diagonal', 'aspectratio_opengate','aspectratio_178','active_sensor_height_178']))
                                      .find(['camera', toCamera])
                                      //.map(o => $lodash.omit(o, ['id', 'camera']))
                                      .value();

        let result = ((focalLength * fromCameraData['crop_factor']) / toCameraData['crop_factor']) * aspectRatio;
        this.output = `<strong class="text-success">Equivalent focal length for ${toCamera}: ` + 
                      parseFloat(Math.round(result * 100) / 100).toFixed(0) + 'mm.</strong>';
      } else {
  ...
  async created() {
    ...
    try {
      const response = await axios.get(`https://api.npoint.io/5c6005c5820933adf98e/SensorSize`, config);
      ...
      this.CameraList = $lodash.chain(response.data)
                            .map(camera => $lodash.omit(camera, [ "sensor_width", "sensor_height", "sensor_diagonal", "aspectratio_opengate", "aspectratio_178", "active_sensor_height_178", "crop_factor"]))
                            .value();
      this.CameraDataList = $lodash.chain(response.data)
                                    .value();
