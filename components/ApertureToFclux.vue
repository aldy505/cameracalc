...
export default {
    ...
    methods: {
        calculate: function() {
            ...
            // validate FC and Lux
            if (iso == "" && aperture == "") {
                this.output = '<strong class="text-danger">Please fill the ISO and Aperture number</strong>'
            } else {
                // Checking the fps
                ...
                if (fc != "") {
                    ...
                    let lux = fc * 10.764
                    this.output = '<strong class="text-success">You need: ' + parseFloat(Math.round(fc * 100) / 100).toFixed(2) + " footcandle and " + parseFloat(Math.round(lux * 100) / 100).toFixed(2) + " lux.</strong>";
                } else {
